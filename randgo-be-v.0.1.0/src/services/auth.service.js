...
const loginUserWithEmailAndPassword = async (email, password) => {
  ...
  if (!user || !(await user.isPasswordMatch(password))) {
    throw new ApiError(httpStatus.UNAUTHORIZED, ErrorMessage.IncorrectEmailAndPassword);
  }
  return user;
};
...
const refreshAuth = async (refreshToken) => {
  try {
    ...
    throw new ApiError(httpStatus.UNAUTHORIZED, ErrorMessage.Authenticate);
  }
};
...
const resetPassword = async (resetPasswordToken, newPassword) => {
  try {
    ...
    throw new ApiError(httpStatus.UNAUTHORIZED, 'Password reset has failed. Please try again later.');
  }
};
