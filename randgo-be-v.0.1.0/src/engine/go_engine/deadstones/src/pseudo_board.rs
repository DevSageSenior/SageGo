...
pub struct PseudoBoard {
    pub data: Vec<Sign>,
    pub edge: Vec<Vec<Vertex>>,
    pub size: usize
}

impl PseudoBoard {
    pub fn new (data : Vec<Sign>, r_edge : Vec<Vertex>, size : usize) -> PseudoBoard {
        let edge = r_edge.split(|&num| num == size).map(|chunk| chunk.to_vec()).collect();
        PseudoBoard {data, edge, size}
    }
