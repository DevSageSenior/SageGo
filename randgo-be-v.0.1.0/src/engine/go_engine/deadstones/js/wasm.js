const make = (wasm) => {
   ...
   * @returns {Uint32Array}
   */
  result.guess = function (data, edge, size, finished, iterations, seed) {
    const retptr = 8;
    const ret = wasm.guess(
      retptr,
      passArray8ToWasm(data),
      WASM_VECTOR_LEN,
      passArray32ToWasm(edge),
      WASM_VECTOR_LEN,
      size,
      finished,
      iterations,
   ...
   */
  result.getProbabilityMap = function (data, edge, size, iterations, seed) {
    const retptr = 8;
    const ret = wasm.getProbabilityMap(
      retptr,
      passArray8ToWasm(data),
      WASM_VECTOR_LEN,
      passArray32ToWasm(edge),
   ...
   * @returns {Int8Array}
   */
  result.playTillEnd = function (data, edge, size, sign, seed) {
    const retptr = 8;
    const ret = wasm.playTillEnd(
      retptr,
      passArray8ToWasm(data),
      WASM_VECTOR_LEN,
      passArray32ToWasm(edge),
   ...
   * @returns {Uint32Array}
   */
  result.getFloatingStones = function (data, edge, size) {
    const retptr = 8;
    const ret = wasm.getFloatingStones(
      retptr,
      passArray8ToWasm(data),
      WASM_VECTOR_LEN,
      passArray32ToWasm(edge),
