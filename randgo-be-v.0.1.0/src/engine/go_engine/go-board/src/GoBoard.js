...
const vertexEquals = (w, v) =>  {
    return w == v
}

class GoBoard {
    constructor(size, edges) {
        this.signMap = new Array(size).fill(0)
        ...
        this._players = [1, -1]
        this._captures = [0, 0]
        this._koInfo = {sign: 0, vertex: [-1]}
    }

    get(x) {
        return this.signMap[x]
    }
    ...
    clear() {
        this.signMap = new Array(this.size).fill(0)
        return this
    }

    /**
     * Make a move on the board
     * @param {Number} sign(-1, 0, 1)
     * @param {Number} vertex 
     * @param {Object} param2 
     * @returns 
     */
    ...
    isValidMove(sign, vertex) {
        const {pass, overwrite, capturing, suicide, ko} = this.analyzeMove(sign, vertex)
        let error = "";
        if(overwrite) error = "Overwrite";
        else if(suicide) error = "Suicide";
        else if(ko) error = "Ko";
        return {isValid : (!overwrite && !suicide && !ko), ErrorMessage : error};
    }
    ...
    clone() {
        let result = new GoBoard(this.size, this.edges)
            .setCaptures(1, this.getCaptures(1))
            .setCaptures(-1, this.getCaptures(-1))
        result.signMap = [...this.signMap]
