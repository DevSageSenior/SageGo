...
1. Clone the repository from the [github](https://amartis/randgo-be)
2. Copy credentials to the project (.env)
3. Execute these commands to install Rust and wasm-pack.
...
4. Get into the project directory and execute following commands.

```bash
rm -rf src/engine/go_engine/deadstones/wasm && wasm-pack build src/engine/go_engine/deadstones --out-dir=./wasm --target=nodejs
```

5. Build a docker image from Dockerfile
...
```bash
npm run restore-minimal-database
```
...
# URL of the Mongo DB
MONGODB_URL=mongodb://127.0.0.1:27017/Go
